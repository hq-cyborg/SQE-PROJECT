...
jest.mock('@/controllers/appControllers', () => ({
  paymentController: {
    ...
    search: jest.fn((req, res) => {
      ...
      const results = query === 'test' ? [{ name: 'Test Payment', amount: 100 }] : [];
...
describe('PAYMENT SEARCH ROUTER API - MOCK TEST', () => {
  let app;
  let mockSearch;
  ...
  beforeEach(() => {
    ...
    mockSearch = appControllers.paymentController.search;
  });
  ...
  it('should return search results for query "test"', async () => {
    const res = await request(app).get('/payment/search').query({ query: 'test' });

    expect(res.statusCode).toBe(200);
    expect(res.body.success).toBe(true);
    expect(res.body.results).toEqual([{ name: 'Test Payment', amount: 100 }]);
  ...
  it('should return empty results for unknown query', async () => {
    const res = await request(app).get('/payment/search').query({ query: 'unknown' });
